...
let B4W (newReversedState: bool) (symbol: SymbolT.Symbol) : SymbolT.Symbol =
    {symbol with ReversedInputPorts = Some newReversedState}

// NB: Prism instead of lens because state is bool option
let reversedPrism = Prism.create reverseStateMux2 B4W
...
/// <summary> Returns the length of an ASegment </summary>
/// <param name="aSegment"> ASegment </param>
/// <returns> Length of ASegment as a float </returns>
let getASegmentLength (aSegment: ASegment) : float =
...
let visibleSegments (wire: Wire): ASegment list =
    wire
...
let visibleSegmentOverlap (vertSeg: ASegment) (horSeg: ASegment) (overlapType: OverlapType) : bool =
    ...
    (improvedOverlap1D (vertStart.X, vertEnd.X) (horStart.X, horEnd.X) overlapType)
    && (improvedOverlap1D (vertStart.Y, vertEnd.Y) (horStart.Y, horEnd.Y) overlapType)
...
let intersectingSymbolPairs (sheet: SheetT.Model) : int =
    let bbs =
        sheet
...
/// <returns> The count over the whole sheet </returns>
let countSymbolIntersectingSegments (model: BusWireT.Model) =
    model
    |> Optic.get (symbol_ >-> symbols_)
    |> mapValuesToList
    |> List.map getSymbolBoundingBox
    |> List.collect (fun bb -> getWiresInBox bb model)
    |> List.distinct
    |> List.length
...
/// <returns> The count over the whole sheet </returns>
let countRightAngleCrosses (model: SheetT.Model) : int =
    ...
    let countWireIntersections (wire1: Wire, wire2: Wire) : int =
        (visibleSegments wire1, visibleSegments wire2)
        ||> List.map2 (fun seg1 seg2 ->
            let OverlapType = match wire1.OutputPort = wire2.OutputPort with
                                   | true -> StrictOverlap
                                   | false -> WeakOverlap
            visibleSegmentIntersect seg1 seg2 OverlapType
        )
        |> List.filter (fun bool -> bool = true)
        |> List.length
    ...
    |> (fun pairs ->
        (0, pairs)
        ||> List.fold (fun count pair -> count + countWireIntersections pair)
    )
...
/// <param name="sheet"> The Sheet model to count over </param>
/// <returns> The count over the whole sheet </returns>
(*
The algorithm being used:
1) Group Horizontal segments by Y, and sort on the start position
2) Group Vertical segments by X, and sort by start position
3)  Apply a removeOverlap fold, since the segments are grouped and sorted, we guarantee that any segments
...
*)
let getVisibleWireLength (sheet: SheetT.Model) =
    ...
    let partitionWiresByOrientation (wires: Wire list) =
        wires
    ...
    let orderByOrientation (visSegments: ASegment list) (ori: Orientation) =
       visSegments
       |>   match ori with
            | Horizontal -> List.groupBy (fun pos -> pos.Start.Y)
            | Vertical -> List.groupBy (fun pos -> pos.Start.X)
       |> List.map (fun (_, segments) ->
           match ori with
           | Horizontal -> List.sortBy (fun pos -> pos.Start.X) segments
           | Vertical -> List.sortBy (fun pos -> pos.Start.Y) segments
       )

    let removeOverlaps (visSegments:  ASegment list list) =
        visSegments
        |> List.collect (fun segments ->
            ([], segments)
    ...
    let getASegmentLength (visSegments: ASegment list) : float =
       (0., visSegments)
       ||> List.fold (fun currentLength seg -> currentLength + squaredDistance seg.Start seg.End)

    sheet
    |> Optic.get SheetT.wire_
    |> getWireNets
    |> List.map partitionWiresByOrientation
    |> List.map (fun (hor,vert) -> orderByOrientation hor Horizontal, orderByOrientation vert Vertical)
    |> List.collect (fun (hor, vert) -> removeOverlaps hor @ removeOverlaps vert)
    |> getASegmentLength
...
/// <param name="model"> The BusWire model of the Sheet </param>
/// <returns> The count over the whole sheet </returns>
let countVisibleRightAngles (model: BusWireT.Model) =
