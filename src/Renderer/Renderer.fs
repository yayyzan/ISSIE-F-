...
let fileMenu (dispatch) =
    ...
    let makeTestItem (name:string) (accelNumber:int)  =
        ...
        makeDebugItem name (Some $"CmdOrCtrl+{accelNumber+1}") (fun _ ->
            dispatch (MenuAction( MenuDrawBlockTest( TestDrawBlockD3.D3Testing.Tests.testMenuFunc, accelNumber), dispatch)))
    makeMenu false "File" [
        ...
        makeMenuGen (debugLevel > 0) false "TESTING" (
            TestDrawBlockD3.D3Testing.Tests.testsToRunFromSheetMenu // make a submenu from this list
        ...
        makeDebugItem "TestModel" (Some "CmdOrCtrl+T") (fun ev ->
            dispatch <| Msg.ExecFuncInMessage(TestDrawBlockD3.D3Testing.Tests.testModelGen,dispatch))
